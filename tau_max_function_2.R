# Function to calculate maximum tau values across different lambda values (alternative version)
# Differences from tau_max: Uses column 2 as response instead of column 1
# Arguments:
...
tau_max_2 <- function(data_list, lambda_seq, alpha) {
  ...
  for (i in seq_along(lambda_seq)) {
    ...
    for (l in 1:L) {
      ...
      # Prepare data: x is all columns except first two (different from tau_max)
      # Note: Comment indicates "we assume last column is response" but implementation uses column 2
      x <- as.matrix(data[, -c(1,2)])
      # Using column 2 as response (different from tau_max which used column 1)
      y <- data[, 2]
